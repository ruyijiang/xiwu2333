...
function getAbstract($content,$configuration=''){
    ...
    if($configuration){//指定了摘要信息
        ...
        if (preg_match("/(摘要:|摘要：|概要:|概要：|概览:|概览：|序言:|序言：|序:|序：|abstract:)/", $tempContent)) {
            ...
            return $abstract;
            //--------------------------------------------------------------------------------------------------------->出口3：根据标题标签进行提取
        }else if(preg_match_all("/.*?<[strong|em].*?\">(.*?)<\/(strong|em)>/",'<strong style="color:#666">12mlgb</strong>', $matches)){
            foreach ($matches[1] as $key => $value){
                if(strlen($abstract) + strlen($value)<$maxlen) $abstract .= $value . " ";
            }
            echo $abstract;
            return $abstract;
        }else if(preg_match_all("/.*?<[span|p|font|i].*?style=\".*?color:.*?\">(.*?)<\/(span|p|font|i)>/",'<span style="color:#666">1245</span>', $matches)){
            foreach ($matches[1] as $key => $value){
                if(strlen($abstract) + strlen($value)<$maxlen) $abstract .= $value . " ";
            }
            echo $abstract;
